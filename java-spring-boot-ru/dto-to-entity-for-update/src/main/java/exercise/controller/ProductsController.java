...
public class ProductsController {
    ...
    public ProductDTO update(@PathVariable long id, @RequestBody ProductUpdateDTO postData) {
        var product = productRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Product with id " + id + " not found"));
        toEntityForUpdate(postData, product);
        productRepository.save(product);
        var productDTO = toDTO(product);
        return productDTO;
    }

    private Product toEntityForUpdate(ProductUpdateDTO postDto, Product product) {
